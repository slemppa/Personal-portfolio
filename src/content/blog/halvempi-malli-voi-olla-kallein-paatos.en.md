...
Latency is another hidden cost. If the agent's p95 latency is 5 seconds, it means that in 95% of cases the response is returned in under 5 seconds. But if the latency rises to 10–15 seconds – for example because of [long context](/blog/sisaltomarkkinointi-ai-kayttotapaukset-pk-yritykselle.en) or complex routing – it slows down the entire sales process. Latency can lead to leads going stale, customers waiting or a campaign's timing going wrong. And if the agent doesn't scale, the cost materializes as lost sales opportunities.
